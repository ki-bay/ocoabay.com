...
export async function onRequest(context) {
  ...
  try {
    ...
    // (1) release capacity for expired holds (session + linked Club House), then delete them
    ...
    await sql`update availability_slots a set held = greatest(0, a.held - h.qty)
      from holds h where h.club_slot_id = a.id and h.expires_at < now()`;
    ...
    const stale = await sql`select r.id, r.slot_id, r.club_slot_id, r.party_size from reservations r
